...
class ResNetStyleEncoder(nn.Module):
    def __init__(self, cfg, d_in, device, dtype):
        super().__init__()
        self.num_latents = cfg.num_latents or d_in * cfg.expansion_factor

        # Main encoder path
        layer_list = []
        for i in range(cfg.num_encoder_layers - 1):
            layer_list.append(
                nn.Linear(
                    d_in if i == 0 else d_in * cfg.hidden_encoder_expansion_factor,
                    d_in * cfg.hidden_encoder_expansion_factor,
                    device=device,
                    dtype=dtype,
                )
            )
            layer_list.append(getattr(nn, cfg.hidden_encoder_act)())
        layer_list.append(
            nn.Linear(
                d_in * cfg.hidden_encoder_expansion_factor,
                self.num_latents,
        ...
        self.main_path = nn.Sequential(*layer_list)

        # Parallel linear layer (skip connection)
        self.skip_connection = nn.Linear(d_in, self.num_latents, device=device, dtype=dtype)
    ...
    def get_weight_init(self):
        return self.skip_connection.weight
...
class NonlinearEncoder(nn.Module):
    ...
    @torch.no_grad()    
    def get_weight_init(self):
        to_set = torch.eye(self.d_in, self.d_in, device=self.this_device, dtype=self.this_dtype)
        for layer in self.layer_list:
            if isinstance(layer, nn.Linear):
                to_set = to_set @ layer.weight.T
        return to_set.T
...
class Sae(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.cfg = cfg
        self.d_in = d_in
        self.num_latents = cfg.num_latents or int(d_in * cfg.expansion_factor)
        self.this_dtype = dtype or torch.float32

        if cfg.num_encoder_layers == 1:
            self.encoder = nn.Linear(d_in, self.num_latents, device=device, dtype=dtype)
            self.encoder.bias.data.zero_()
            to_set = self.encoder.weight
        else:
            if self.cfg.use_both:
                self.encoder = ResNetStyleEncoder(cfg, d_in, device, dtype)
            else:
                self.encoder = NonlinearEncoder(cfg, d_in, device, dtype)
            to_set = self.encoder.get_weight_init()
            
        print(self.encoder, to_set.shape)

        self.W_dec = nn.Parameter(to_set.clone().contiguous()) if decoder else None
    ...
    def pre_acts(self, x: Tensor) -> Tensor:
        ...
        sae_in = x.to(self.this_dtype) - self.b_dec
    ...
    def decode(self, top_acts: Tensor, top_indices: Tensor) -> Tensor:
        ...
        y = decoder_impl(top_indices, top_acts.to(self.this_dtype), self.W_dec.mT)
        return y + self.b_dec
