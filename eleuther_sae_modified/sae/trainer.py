...
class SaeTrainer:
    def __init__(
        self, cfg: TrainConfig, dataset: HfDataset | MemmapDataset, model: PreTrainedModel, hidden_modification=None
    ):
        self.hidden_modification = hidden_modification
        if cfg.hookpoints:
    ...
    def fit(self):
        ...
        for batch in dl:
            ...
            for name, hiddens in hidden_dict.items():
                if self.hidden_modification is not None:
                    with torch.no_grad():
                        hiddens = self.hidden_modification(hiddens)
                ...
                if self.global_step == 0:
                    ...
                    raw.b_dec.data = median.to(raw.this_dtype)
    ...
    def save(self):
        ...
        path = f"sae-ckpts/{self.cfg.run_name}" or "sae-ckpts"
        rank_zero = not dist.is_initialized() or dist.get_rank() == 0
